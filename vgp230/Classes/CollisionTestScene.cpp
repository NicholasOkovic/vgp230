...
bool CollisionTestScene::init()
{
  ...
  mushroom->addComponent(CollisionComponent::createCircle((mushroom->getBoundingBox().size.width)/2));
  /// TODODONE: ADD CODE Here -- Requires CollisionComponent

  bullet2 = Sprite::create("bullet2.png");
  bullet2->setPosition(Vec2(visibleSize.width / 8.0f, visibleSize.height / 4.0f));
  bullet2->addComponent(CollisionComponent::createCircle((bullet2->getContentSize().height)/2));
  /// TODODONE: ADD CODE Here -- Requires CollisionComponent

  hummer = Sprite::create("hummer.png");
  hummer->setPosition(Vec2(7.0f * visibleSize.width / 8.0f, visibleSize.height / 4.0f));
  hummer->addComponent(CollisionComponent::createBox(hummer->getContentSize().width, (hummer->getContentSize().height)/2));
  /// TODODONE: ADD CODE Here -- Requires CollisionComponent
...
{
    if (b._x + b._width < p.x || b._x > p.x + 0)
        return false;

    if (b._y + b._height < p.y || b._y > p.y + b._height)
        return false;

  /// TODODONE: ADD CODE Here
  return true;
}
...
bool CollisionComponent::checkCollision(const Vec2& p, const Vec2& c, float r)
{
    ...
    if (( ( (p.x - c.x) * (p.x - c.x) ) + ( (p.y - c.y) * (p.y - c.y) )) <= ( (r + 0) * (r + 0) ))//squared could be cleaner ask teacher
    {
        return true;
    }
...
bool CollisionComponent::checkCollision(const Vec2& c1, float r1, const Vec2& c2, float r2)
{
    ...
    if ((( (c1.x - c2.x) * (c1.x - c2.x) ) + ( (c1.y - c2.y) * (c1.y - c2.y) )) <= ( (r1 + r2) * (r1 + r2) ))//squared could be cleaner ask teacher
    {
        return true;
    }
  /// TODODONE: ADD CODE Here
...
{
    auto aabbCenter = Vec2((b1._x + ( b1._width / 2.0f)), (b1._y + (b1._height / 2.0f)));

  return checkCollision(aabbCenter, c2, r2) || /// center of rectangle is in circle
