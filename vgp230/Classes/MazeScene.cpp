...
void MazeScene::setPosition(Sprite* spr, std::pair<int, int> newPosition, std::pair<int, int>& oldPosition, bool draw)
{
  ...
  //spr->setPosition(newPosition.first, newPosition.second);
  ...
  if (draw)
  {
    /// TODO: Add drawing of rectangle around "spr"
      Vec2 destination = spr->getPosition() + Vec2(spr->getContentSize().width, spr->getContentSize().height);

      drawNode->drawRect(spr->getPosition(),destination, Color4F::RED);
